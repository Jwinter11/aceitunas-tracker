...
if active_page == "Resumen":
    ...
    if _ult_p and _pen_p:
        _avg_u = dff[dff["Periodo"] == _ult_p].groupby(["Cadena", "SKU_canonico"])["Precio_100g"].mean()
        _avg_p = dff[dff["Periodo"] == _pen_p].groupby(["Cadena", "SKU_canonico"])["Precio_100g"].mean()
    ...
    with st.expander("🔔 Novedades", expanded=True):
            ...
            with _cn_l:
                st.markdown('<div class="chart-note">📊 Cambios de precio vs semana anterior ($/100g)</div>',
                            unsafe_allow_html=True)
                if not _cambios:
